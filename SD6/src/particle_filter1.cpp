...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	for (unsigned int i = 0; i < observations.size(); i++) {
		...
		for (unsigned int j = 0; j < predicted.size(); j++) {
			...
			if (min_dist > 10.0) {
				std::cout << "Distance too large " << min_dist << std::endl;
			}
		}
	}
}
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for (int i = 0; i < this->num_particles; i++) {
		...
		for (unsigned int i = 0; i < transformedObs.size(); i++) {
			for (unsigned int j = 0; j < predictedObs.size(); j++) {
				if (transformedObs[i].id == predictedObs[j].id) {
					double x_obs = transformedObs[i].x;
					double y_obs = transformedObs[i].y;
					double mu_x = predictedObs[j].x;
					double mu_y = predictedObs[j].y;
